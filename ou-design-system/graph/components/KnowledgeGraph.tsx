...
import { Box, Stack, Text, Slider, Switch, Group, ActionIcon, Tooltip, TextInput, Collapse, UnstyledButton } from '@mantine/core';
...
export function KnowledgeGraph({ nodes, edges, onNodeClick, hideSettingsButton, settingsOpen: externalSettingsOpen, onSettingsToggle }: KnowledgeGraphProps) {
  ...
  const [internalControlsOpen, { toggle: internalToggle, close: closeControls }] = useDisclosure(false);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      app.canvas.style.borderRadius = 'var(--mantine-radius-md)';
      ...
      labelCvs.style.borderRadius = 'var(--mantine-radius-md)';
  ...
  return (
    <Box style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {!hideSettingsButton && (
        <Tooltip label="그래프 설정">
          <ActionIcon variant="subtle" color="gray" size="md" onClick={toggleControls}
            style={{ position: 'absolute', top: 56, right: 16, zIndex: 21,
              background: isDark ? 'rgba(10,12,20,0.7)' : 'rgba(255,255,255,0.8)', backdropFilter: 'blur(8px)' }}>
            <GearSix size={16} />
          </ActionIcon>
        </Tooltip>
      )}
      ...
      {editMode && (
        <Box style={{
          ...
          borderRadius: 'var(--mantine-radius-md)', padding: '6px 14px', backdropFilter: 'blur(8px)',
        }}>
          <Text fz={11} c={isDark ? 'cyan' : 'teal'} ta="center">
      ...
      {contextMenu && (
        <Box style={{
          ...
          borderRadius: 'var(--mantine-radius-md)', backdropFilter: 'blur(12px)',
          padding: 4, minWidth: 160,
        }}
          onMouseLeave={() => setContextMenu(null)}>
          <UnstyledButton
            onClick={() => handleAddNodeAsAttractor(contextMenu.nodeId, false)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 4, width: '100%' }}
            className="hover-bg">
            <Plus size={12} color={isDark ? '#00CED1' : '#008B8B'} />
            <Text fz={11}>중력점으로 추가</Text>
          </UnstyledButton>
          <UnstyledButton
      ...
      {controlsOpen && (
        <Box style={{
          ...
          border: '0.5px solid var(--mantine-color-default-border)',
          borderRadius: 'var(--mantine-radius-md)', backdropFilter: 'blur(12px)', zIndex: 10,
        }}>
          <Box p={10} pb={0}>
            <TextInput size="xs" placeholder="검색..." leftSection={<MagnifyingGlass size={12} />}
              value={searchQuery} onChange={(e) => setSearchQuery(e.currentTarget.value)}
              styles={{ input: { fontSize: 11, height: 30, borderRadius: 'var(--mantine-radius-sm)' } }} />
          </Box>
          <Box px={10} pt={12}>
          ...
          <Collapse in={displayOpen}>
            <Stack gap={10} px={10} pb={12} pt={8}>
              <Group justify="space-between"><Text fz={11}>라벨</Text><Switch size="xs" checked={showLabels} onChange={(e) => setGS('showLabels', e.currentTarget.checked)} /></Group>
              <Group justify="space-between"><Text fz={11}>글로우</Text><Switch size="xs" checked={showGlow} onChange={(e) => setGS('showGlow', e.currentTarget.checked)} /></Group>
              <Box><Text fz={11} mb={4}>텍스트 표시 시작점</Text><Slider size="xs" min={1} max={10} step={0.5} value={labelZoom} onChange={(v) => setGS('labelZoom', v)} styles={{ root: { padding: 0 } }} /></Box>
              <Box><Text fz={11} mb={4}>노드 크기</Text><Slider size="xs" min={0.3} max={4} step={0.1} value={nodeSize} onChange={(v) => setGS('nodeSize', v)} styles={{ root: { padding: 0 } }} /></Box>
              <Box><Text fz={11} mb={4}>링크 두께</Text><Slider size="xs" min={0.1} max={2} step={0.1} value={linkThickness} onChange={(v) => setGS('linkThickness', v)} styles={{ root: { padding: 0 } }} /></Box>
              <Group justify="space-between">
                <Text fz={11}>노드 색상</Text>
                <Box style={{ position: 'relative', width: 22, height: 22 }}>
                  <Box style={{ width: 22, height: 22, borderRadius: '50%', background: nodeColor, border: '1.5px solid var(--mantine-color-default-border)' }} />
              ...
              <Group justify="space-between"><Text fz={11}>모드별 자동 조정</Text><Switch size="xs" checked={adaptiveColor} onChange={(e) => setGS('adaptiveColor', e.currentTarget.checked)} /></Group>
            </Stack>
          </Collapse>
          ...
          <Collapse in={forceOpen}>
            <Stack gap={10} px={10} pb={12} pt={8}>
              <Group justify="space-between"><Text fz={11}>중력장 편집</Text><Switch size="xs" checked={editMode} onChange={(e) => { setEditMode(e.currentTarget.checked); setContextMenu(null); if (!e.currentTarget.checked) setSelectedAttractorId(null); }} /></Group>
              {editMode && selectedAttractorId && (() => {
                const selAtt = attractorsRef.current.find(a => a.id === selectedAttractorId);
                if (!selAtt) return null;
                return (
                  <Box style={{ background: isDark ? 'rgba(0,206,209,0.08)' : 'rgba(0,139,139,0.06)', borderRadius: 'var(--mantine-radius-sm)', padding: '6px 8px' }}>
                    <Text fz={10} c={isDark ? 'cyan' : 'teal'} fw={600} mb={4}>
                      선택된 중력점{selAtt.linkedNodeId ? ` (${selAtt.linkedNodeId})` : ''}
                    </Text>
                    <Box mb={6}>
                      <Text fz={11} mb={4}>개별 중력 강도</Text>
                      <Slider size="xs" min={0} max={0.3} step={0.005} value={selAtt.strength}
                        onChange={(v) => {
                          selAtt.strength = v;
                          workerRef.current?.postMessage({ type: 'updateAttractorStrength', data: { attractorId: selectedAttractorId, strength: v } });
                          renderFrame();
                        }}
                    ...
                    <Group justify="space-between">
                      <Text fz={11}>편집 후 표시</Text>
                      <Switch size="xs" checked={selAtt.visible === true}
                        onChange={(e) => {
              ...
              <Box><Text fz={11} mb={4}>중심 장력 (전체)</Text><Slider size="xs" min={0} max={0.3} step={0.01} value={centerForce} onChange={(v) => setGS('centerForce', v)} styles={{ root: { padding: 0 } }} /></Box>
              <Box><Text fz={11} mb={4}>반발력</Text><Slider size="xs" min={1} max={100} value={repulsion} onChange={(v) => setGS('repulsion', v)} styles={{ root: { padding: 0 } }} /></Box>
              <Box><Text fz={11} mb={4}>링크 거리</Text><Slider size="xs" min={5} max={150} value={linkDistance} onChange={(v) => setGS('linkDistance', v)} styles={{ root: { padding: 0 } }} /></Box>
              <Box><Text fz={11} mb={4}>링크 장력</Text><Slider size="xs" min={0.01} max={1} step={0.01} value={linkStrength} onChange={(v) => setGS('linkStrength', v)} styles={{ root: { padding: 0 } }} /></Box>
            </Stack>
          </Collapse>
          <Box style={{ borderTop: '0.5px solid var(--mantine-color-default-border)' }} />
          <Group justify="space-between" px={10} py={8}>
            {nodeCount && <Text fz={9} c="dimmed">{nodeCount}</Text>}
            <Group gap={4}>
              <Tooltip label="초기화"><ActionIcon size="xs" variant="subtle" color="gray" onClick={() => gs.reset()}><ArrowCounterClockwise size={12} /></ActionIcon></Tooltip>
              <Tooltip label="전체 보기"><ActionIcon size="xs" variant="subtle" color="gray" onClick={handleZoomToFit}><ArrowsIn size={12} /></ActionIcon></Tooltip>
            </Group>
          </Group>
        </Box>
      )}
