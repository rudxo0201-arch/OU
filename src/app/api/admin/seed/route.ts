...
import { seedHanjaData } from '@/lib/seed/hanja-seed';
import { seedShanghanlunData } from '@/lib/seed/shanghanlun-seed';

export const maxDuration = 300; // 한자 전체 시딩 시 5분 필요
...
export async function POST(request: Request) {
  try {
    ...
    if (type === 'hanja' || type === 'hanja-all' || type === 'hanja-radicals') {
      const scope = type === 'hanja-all' ? 'all'
        : type === 'hanja-radicals' ? 'radicals'
        : 'graded';
      const result = await seedHanjaData(supabase, scope as any);
      return NextResponse.json({
        success: true,
        created: result.created,
        skipped: result.skipped,
        message: result.created > 0
          ? `한자 데이터 ${result.created}개 생성 (${scope}).`
          : '이미 모든 한자 데이터가 존재합니다.',
      });
    }
    ...
    if (type === 'shanghanlun') {
      ...
      return NextResponse.json({
        ...
        skipped: result.skipped,
        message: result.created > 0
          ? `상한론 조문 ${result.created}개 생성.`
          : '이미 모든 상한론 데이터가 존재합니다.',
      });
    }
