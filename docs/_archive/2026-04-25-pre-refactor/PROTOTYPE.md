...
> **LEGACY 마킹 ≠ 자동 삭제.**
> 마킹은 "이 코드는 낡은 컨셉이니 제거 의지가 있다"는 표시일 뿐, 실제 삭제 시점은 별도 단계다.
> 특히 **사용자 데이터가 얽혀 있으면 이관 없이 삭제 금지**.

파일 상단에 두 줄로 기재:

```ts
// LEGACY(YYYY-MM-DD): <이전 컨셉명>. <대체 경로/Phase>에서 제거 예정.
// DATA: <none | localStorage="<key>" | supabase:<table> | R2:<bucket>> — <이관 전략 또는 'no-op'>
```

**DATA 필드 규칙:**
- `none` — 순수 UI 코드. 데이터 의존 0. 삭제 시 이관 불필요.
- `localStorage="key"` — 브라우저 로컬 상태. 삭제 전 키 내용 검토. 사용자 설정이면 새 구조로 마이그레이션, 임시값이면 버려도 됨.
- `supabase:table` — DB 테이블 참조. 삭제 전 해당 테이블 사용처 전수 조사. 다른 경로가 같은 테이블을 쓰면 데이터는 그대로 둔다. 이 파일만 쓴다면 마이그레이션 스크립트 필요.
- `R2:bucket` — 원본 파일. **절대 자동 삭제 금지**. R2 원본 보존은 OU 불변 원칙.

**삭제 전 체크리스트 (LEGACY 주석이 붙은 파일 제거 시):**
1. DATA 필드에 쓰인 저장소 확인
2. 데이터 사용자 수/총량 확인 (쿼리 또는 로그)
3. 이관 스크립트 작성 + 소규모 테스트
4. 이관 실행 → 소스 코드 삭제 → 저장소 정리 (순서 엄수)
5. PR 설명에 "이관된 데이터 건수 + 롤백 방법" 명시

**예시 (`/my/page.tsx`):**
```ts
// LEGACY(2026-04-24): 구 홈(위젯 바탕화면) 컨셉. /home 통합 + Phase 2에서 폐기 예정.
// DATA: localStorage="ou.widget-store" — Phase 2에서 /home 위젯 레이아웃으로 마이그레이션 필요.
```
